...
import { gradeLocal } from "../../engine/grade_cli";
...
export function LessonView({ seed }:{ seed:{ original_text:string; design_translation:string; examples:string[]; concept:string; prompt:string; requires_vocabulary:string[]; } }) {
  ...
  const [feedback,setFeedback] = useState<ReturnType<typeof gradeLocal> | null>(null);
  const [nextHint,setNextHint] = useState<string>("");

  const runGrade = () => {
    const result = gradeLocal(draft, intent, seed.concept, seed.requires_vocabulary);
    setFeedback(result);
    setNextHint(result.nextHint || "Keep the line that shifts the air.");
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 p-4">
      ...
      <div className="space-y-4">
        ...
        {nextHint && <p className="text-xs text-slate-600">Next hint: {nextHint}</p>}
      </div>

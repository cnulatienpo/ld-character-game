import { analyzeSignals } from "./heuristics.js";
import { speak } from "./dialect.js";
import { suggestNextUnlocks } from "./gating.js";

const strengthFromSignals = (input, signals) => {
  const behaviorKey = input.behavior;
  const behaviorHit = Boolean(signals[behaviorKey]);
...
const describeEvidence = (key, evidence) => {
...
const selectManeuvers = (behavior, target, computed) => {
...
const buildFeedback = (input, strength, signals, maneuvers, nextUnlocks) => {
  ...
  if (!signals.pressure || !signals.follow_through) {
    sentences.push("Keep tracing the cost and response so the consequence line reads clearly.");
...
export const grade = (input, seeder) => {
  ...
  Object.keys(report).forEach((key) => {
    if (report[key] === 1) {
      const line = describeEvidence(key, evidence);
      if (line) {
  ...
  const nextUnlocks = suggestNextUnlocks(input, report, seeder);

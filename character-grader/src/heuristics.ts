import { SignalReport } from "./types.js";
...
const SHOW_VERBS = [
  "push", "grab", "slam", "shove", "pace", "nod", "glance", "stare",
  "clench", "tremble", "flinch", "fidget", "shrug", "smirk", "smile",
  "grin", "grimace", "sigh", "huff", "storm", "step", "lean", "fold"
];

const SENSORY_TERMS = [
  "smell", "sweat", "heartbeat", "pulse", "taste", "sound", "echo",
  "heat", "cold", "chill", "buzz", "sting"
];

const OBJECT_CUES = [
  "keys", "door", "phone", "screen", "hoodie", "ring", "knife",
  "coffee", "table", "receipt", "bus", "train", "stairs", "locker"
];
...
const SPECIFIC_NOUNS = [
  "door", "keys", "table", "receipt", "hoodie", "bus", "knife", "ring",
  "coffee", "locker", "alley", "cashier", "subway", "poster", "stairs",
  "bench", "notebook", "badge", "mirror", "couch"
];
...
const findMatches = (patterns: (RegExp | string)[], text: string): string[] => {
...
const countSpecificNouns = (tokens: string[]): number => {
  const set = new Set(SPECIFIC_NOUNS);
  const matches = tokens.filter((token) => set.has(token));
  return unique(matches).length;
};

const hasNumbersOrProperNouns = (text: string): boolean => {
  if (/\b\d+\b/.test(text)) {
    return true;
  }
  // Very lightweight proper noun detector: capitalized word not at sentence start.
  return /(\.|!|\?)\s+[A-Z][a-z]+/.test(text);
};

const detectSay = (text: string, evidence: SignalEvidence): 0 | 1 => {
...
const detectShow = (text: string, tokens: string[], evidence: SignalEvidence): 0 | 1 => {
  const matches: string[] = [];
...
const detectHide = (text: string, evidence: SignalEvidence): 0 | 1 => {
...
const detectPressure = (text: string, evidence: SignalEvidence): 0 | 1 => {
...
const detectFollowThrough = (
  ...
  let actionShift = 0;
  // naive detection: look for verbs in past tense after connector
  ...
  // fallback: look for sequential verbs
...
const detectFieldChange = (text: string, evidence: SignalEvidence): 0 | 1 => {
...
export const analyzeSignals = (
  text: string
): { report: SignalReport; evidence: SignalEvidence } => {
